 ...
 * x, y and z.
 */
static s32 envfx_is_snowflake_alive(struct EnvFxParticle *particle, s32 snowCylinderX,
                                    s32 snowCylinderY, s32 snowCylinderZ) {
    s32 x = particle->xPos;
    s32 y = particle->yPos;
    s32 z = particle->zPos;
...
void envfx_update_snow_normal(s32 snowCylinderX, s32 snowCylinderY, s32 snowCylinderZ) {
    ...
    s16 deltaX_div = (s16) (deltaX / 1.2);
    s16 deltaY_div = (s16) (deltaY * 0.8);
    s16 deltaZ_div = (s16) (deltaZ / 1.2);
    s16 deltaX_2 = (s16) (deltaX * 2);
    s16 deltaZ_2 = (s16) (deltaZ * 2);

    struct EnvFxParticle *particle = gEnvFxBuffer;

    for (i = 0; i < gSnowParticleCount; i++, particle++) {
        particle->isAlive =
            envfx_is_snowflake_alive(particle, snowCylinderX, snowCylinderY, snowCylinderZ);
        if (!particle->isAlive) {
            particle->xPos = 400.0f * random_float() - 200.0f + snowCylinderX + deltaX_2;
            particle->zPos = 400.0f * random_float() - 200.0f + snowCylinderZ + deltaZ_2;
            particle->yPos = 200.0f * random_float() + snowCylinderY;
...
void envfx_update_snow_blizzard(s32 snowCylinderX, s32 snowCylinderY, s32 snowCylinderZ) {
    ...
    for (i = 0; i < gSnowParticleCount; i++, particle++) {
        ...
        if (!particle->isAlive) {
            particle->xPos = 400.0f * random_float() - 200.0f + snowCylinderX + deltaX_2;
            particle->zPos = 400.0f * random_float() - 200.0f + snowCylinderZ + deltaZ_2;
            particle->yPos = 400.0f * random_float() - 200.0f + snowCylinderY;
            particle->isAlive = TRUE;
        } else {
            particle->xPos += random_float() * 2 - 1.0f + deltaX_div + 20.0f;
            particle->yPos -= 5 - deltaY_div;
            particle->zPos += random_float() * 2 - 1.0f + deltaZ_div;
        }
    }
...
void envfx_update_snow_water(s32 snowCylinderX, s32 snowCylinderY, s32 snowCylinderZ) {
    ...
    for (i = 0; i < gSnowParticleCount; i++, particle++) {
        particle->isAlive =
            envfx_is_snowflake_alive(particle, snowCylinderX, snowCylinderY, snowCylinderZ);
        if (!particle->isAlive) {
            particle->xPos = 400.0f * random_float() - 200.0f + snowCylinderX;
            particle->zPos = 400.0f * random_float() - 200.0f + snowCylinderZ;
            particle->yPos = 400.0f * random_float() - 200.0f + snowCylinderY;
            particle->isAlive = TRUE;
        }
    }
